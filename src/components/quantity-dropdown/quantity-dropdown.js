...
class Quantity {
    ...
    normalize(value) {
        const inputSize = Math.floor(parseInt(this.viewInput.offsetWidth) * 0.125);
        const arrayFromValue = value.split(', ');

        return arrayFromValue.reduce((acc, str) => (`${acc} ${str},`.length < inputSize) ? `${acc} ${str},` : `${acc}`, '');
    }
    ...
    updateViewValue() {
        ...
        const inputValueNormalized = this.normalize(inputValue.substr(0, inputValue.length - 1));

        this.viewInput.value = inputValueNormalized.substr(0, inputValueNormalized.length - 2);
